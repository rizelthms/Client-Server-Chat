...
class SingleUserTests {
    ...
    @Test
    void tc58BroadcastReturnsErrorWhenUserNotLoggedIn(TestReporter testReporter) throws JsonProcessingException {
        receiveLineWithTimeout(in); //ready message
        out.println(Utils.objectToMessage(new BroadcastReq("test")));
        out.flush();
        String response = receiveLineWithTimeout(in);
        BroadcastResp broadcastResponse = Utils.messageToObject(response);

        assertEquals(new BroadcastResp("ERROR", 6000), broadcastResponse);
    }

    @Test
    void tc59HangupIsReceivedIfNoPongIsSent() throws IOException {
        receiveLineWithTimeout(in); //ready message
        out.println(Utils.objectToMessage(new Enter("myname")));
        out.flush();
        receiveLineWithTimeout(in); //server response
        ...
        in.readLine();
        // hangup
        String hangupString = in.readLine();
        Hangup pongError = Utils.messageToObject(hangupString);
        assertEquals(new Hangup(7000), pongError);
    }
